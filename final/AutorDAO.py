...
class AutorDAO:
    ...
    def inserir_autor2(self,Autor, Musica):
        ...
        try:
            ...
            query_condicional = "SELECT * FROM autor WHERE id_autor = %s"

            data_condicional = Autor.id_autor
    ...
    def consulta_autor(self,nome):

        try:
            query1 = ("SELECT * FROM autor WHERE nome = %s")

            data1 = nome

            cursor = self.conexao.cursor(buffered=True)
            cursor.execute(query1, (data1,))
            for (id_autor, nome) in cursor:
                print("{},{}".format(
                    id_autor, nome))
